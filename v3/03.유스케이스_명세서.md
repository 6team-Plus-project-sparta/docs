...
| UC-011 | 선착순 쿠폰 발급 | User | 분산락 (쿠폰) | 이도현 |
...
- **주요 흐름**:
  ...
  2. 시스템이 해당 사용자의 현재 Hold 수를 확인한다. (`user-holds:{userId}` Set 크기 조회)
  ...
  5. 좌석이 AVAILABLE이면 Redis에 Hold 키를 SET한다. (`hold:{eventId}:{seatId}` → userId, TTL 5분)
  6. `user-holds:{userId}` Set에 해당 좌석을 추가한다.
...
- **비고 — Hold 수 확인의 Race Condition**:
  > 주요 흐름 2번의 `SCARD user-holds:{userId}` 는 분산락 **바깥**에서 수행된다. 극단적 동시 요청 시 두 요청이 동시에 "3 < 4"로 판단해 5석째 Hold가 생길 가능성이 있다. 최종 방어선은 `user-holds:{userId}` Set의 크기 제약을 Lua Script로 확인 후 SADD하는 원자적 구현이며, 이 부분은 FN-SEAT-02 구현 시 함께 반영한다.
- **관련 필수/도전 기능**:
  - 필수 — 좌석 임시 점유 (Hold), 동시성 제어 (Lettuce SETNX), 어뷰징 방지
  - 도전 — Redisson 전환
...
- **주요 흐름**:
  1. 사용자가 쿠폰 발급 페이지에서 "쿠폰 받기" 버튼을 클릭한다.
     1-1. 시스템이 UserCoupon 테이블에서 `(userId, couponId)` 존재 여부를 확인한다.
  2. 시스템이 Redis 분산락을 획득한다. (`lock:coupon:{couponId}`, Retry with backoff 최대 3회)
  3. 락 내부에서 쿠폰 잔여 수량을 조회한다.
  4. 잔여 수량이 1 이상이면 수량을 -1 차감한다.
  5. 해당 사용자에게 쿠폰을 발급한다. (USER_COUPON 레코드 INSERT, status=ISSUED)
  6. 분산락을 해제한다.
  7. "쿠폰이 발급되었습니다!" 응답을 반환한다.
- **대체 흐름**:
  - 1-1a. 이미 발급받은 쿠폰인 경우 → 락 획득 전 즉시 409 "이미 발급받은 쿠폰입니다." (락 불필요)
  - 3a. 잔여 수량이 0인 경우 → 락 해제 후 409 "쿠폰이 모두 소진되었습니다."
- **예외 흐름**:
  - 2a. 분산락 3회 Retry 후에도 획득 실패 → "요청이 많아 처리할 수 없습니다. 잠시 후 다시 시도해주세요." (503 Service Unavailable)
- **사후조건**: 쿠폰 잔여 수량이 정확히 1 차감되고, 사용자가 해당 쿠폰을 보유한다. 100장 한정이면 정확히 100명에게만 발급된다.
- **관련 필수/도전 기능**:
  - 필수 — 선착순 쿠폰 발급, 동시성 제어 (Lettuce SETNX, Retry with backoff)
  - 도전 — Redisson 전환
