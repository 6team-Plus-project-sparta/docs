...
## ADR-014. 이벤트 ENDED 상태 전환 방식 — 스케줄러 자동화
...
| **상태** | 승인됨 |
| **결정자** | 팀원 B |
| **관련 문서** | 기능 명세서 FN-EVT-04, ERD v7.0 §2-4 |

### 컨텍스트

공연 종료 후 이벤트 상태를 `ENDED`로 전환하는 방식을 결정해야 한다.
`eventDate`가 경과했음에도 `ON_SALE` 상태가 유지되면 사용자에게 잘못된 정보를 노출할 수 있다.

### 비교

| 방식 | 설명 | 장점 | 단점 |
|------|------|------|------|
| **A안: 수동 전환** | 관리자가 직접 status 변경 | 구현 단순 | 운영 부담, 누락 가능성 |
| **B안: 스케줄러 자동화** | `@Scheduled`로 매 정각 일괄 UPDATE | 자동화, 운영 부담 없음 | 스케줄러 구현 필요 |
| **C안: 조회 시점 동적 판단** | DB는 그대로, 응답 DTO에서 계산 | DB 변경 없음 | DB 상태와 응답 불일치, 캐시 오염 가능 |

### 결정

**B안 채택 — `@Scheduled(cron = "0 0 * * * *")` 매 정각 일괄 UPDATE**

### 이유

1. **운영 안정성**: 관리자 수동 처리 누락 없이 자동으로 상태 일관성 유지
2. **DB 정합성**: C안과 달리 DB 상태와 API 응답이 항상 일치
3. **캐시 연동**: 스케줄러 실행 시 `@CacheEvict`로 `event-detail` 캐시 동시 무효화
4. **구현 단순**: `@Modifying` JPQL 벌크 UPDATE 1개로 처리, 복잡도 낮음
5. **3주 일정 적합**: 스케줄러 구현 비용이 낮아 일정 내 처리 가능
...
|------|------|
| 매분 | 과도한 DB 부하, 불필요 |
| **매 정각** | ✅ `eventDate` 정밀도가 분 단위이므로 충분, DB 부하 최소 |
| 매일 자정 | ENDED 전환이 최대 23시간 지연 가능 |

### 트레이드오프

| 상황 | 대응 |
|------|------|
| 정각 직후 조회 시 최대 1시간 오차 | `eventDate` 기준 실시간 판단이 필요하면 C안 병행 고려 |
